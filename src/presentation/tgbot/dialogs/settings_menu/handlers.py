...
async def on_click_update_language(
    ...
    await update_language(UserDTO(tg_id=dialog_manager.event.from_user.id, language=language))
...
async def on_click_update_notify(
    ...
    await query.answer(l10n.format_value('on-msg') if flag else l10n.format_value('off-msg'))
