...
class DatabaseProvider(Provider):
    ...
    async def get_async_engine(self, config: DatabaseConfig) -> AsyncIterable[AsyncEngine]:
    ...
    @provide(scope=Scope.APP)
    async def get_session_maker(self, async_engine: AsyncEngine) -> AsyncIterable[async_sessionmaker]:
        yield async_sessionmaker(
            bind=async_engine, autoflush=False, expire_on_commit=False
        )

    @provide(scope=Scope.REQUEST)
    async def get_session(self, async_session_maker: async_sessionmaker) -> AsyncIterable[AsyncSession]:
